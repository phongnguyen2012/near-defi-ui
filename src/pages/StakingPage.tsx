...
function StakingPage() {
    ...
    const handleStakeToken = async () => {
        ...
        try {
            await stakeToken(parseTokenAmount(stakeValue, getTokenMetadata("TUNC").decimals).toLocaleString('fullwide', {useGrouping:false}))
        } catch (e) {
            console.log("Error", e);
        } finally {
    ...
    const handleUnstakeToken = async () => {
        if (!wallet.isSignedIn()) await login();
      if (!unstakeValue || unstakeValue <= 0 || unstakeValue > parseTokenWithDecimals(stakingAccount.stakeBalance, getTokenMetadata("TUNC").decimals)) return;
      setUnstakeLoading(true);
      try {
          await unstake(parseTokenAmount(unstakeValue, getTokenMetadata("TUNC").decimals).toLocaleString('fullwide', {useGrouping:false}))
      } catch (e) {
          console.log("Error", e);
      } finally {
    ...
    const handleClaimReward = async () => {
        if (!wallet.isSignedIn()) await login();
        if (parseTokenWithDecimals(stakingAccount.reward, getTokenMetadata("TUNC").decimals) < 1) return;
                ...
                <div className={"flex flex-col mt-5 justify-between"}>
                    ...
                    <div className={"flex flex-col justify-between mb-2 bg-cardBg rounded-2xl p-5 w-full"}>
                        ...
                        <div className="flex flex-row justify-between">
                            ...
                            <span className="text-sm text-white font-bold">{formatNumber(parseTokenWithDecimals(stakingAccount.stakeBalance, getTokenMetadata("TUNC").decimals))} TUNC</span>
                        </div>
                        <div className="flex flex-row justify-between mb-3">
                            <span className="text-sm text-primaryText">Pending Reward</span>
                            <span className="text-sm text-white font-bold">{formatNumber(parseTokenWithDecimals(stakingAccount.reward, getTokenMetadata("TUNC").decimals))} TUNC</span>
                        </div>
                        <p className="text-xs text-primaryText mb-1"><span>You can claim reward when reward balance greater than 1 TUNC</span></p>
                        <MyButton onClick={handleClaimReward} loading={claimLoading} disable={parseTokenWithDecimals(stakingAccount.reward, getTokenMetadata("TUNC").decimals) < 1} text="Claim Reward"/>
                    </div>
                            ...
                            <TabPane key="stake">
                                <div className={"input-form mt-5"}>
                                    <p className="flex flex-row items-center text-primaryText mb-2">
                                        ...
                                        <img className="mr-1 ml-2" style={{width: 15, height: 15}} src={getTokenMetadata("TUNC").icon} alt=""/><span className="text-primary">TUNC</span>
                                    </p>
                            ...
                            <TabPane key="unstake">
                                <div className={"input-form mt-5"}>
                                    <p className="flex flex-row items-center text-primaryText mb-2">
                                        ...
                                        {formatNumber(parseTokenWithDecimals(stakingAccount.stakeBalance, getTokenMetadata("TUNC").decimals))}
                                        <img className="mr-1 ml-2" style={{width: 15, height: 15}} src={getTokenMetadata("TUNC").icon} alt=""/><span className="text-primary">TUNC</span>
                                    </p>
                                    <InputNumber min={0} className={"staking-input font-bold mb-4 rounded"} addonAfter={<MaxButton onClick={() => setUnstakeValue(parseTokenWithDecimals(stakingAccount.stakeBalance, getTokenMetadata("TUNC").decimals))} />} value={unstakeValue} onChange={(value) => setUnstakeValue(value)} defaultValue={0} />
                                    ...
                                    <MyButton onClick={handleUnstakeToken} loading={unstakeLoading} disable={
                                      !unstakeValue || unstakeValue <= 0 || unstakeValue > parseTokenWithDecimals(stakingAccount.stakeBalance, getTokenMetadata("TUNC").decimals)
                                    } text="Unstake"/>
                                </div>
                            </TabPane>
                        </Tabs>
...

                    {
                        ...
                        <div className={"flex flex-col justify-between bg-cardBg rounded-2xl p-5 w-full"}>
                            ...
                            <div className="flex flex-row justify-between mb-0.5">
                                ...
                                <span className="text-sm text-white font-bold">{formatNumber(parseTokenWithDecimals(stakingAccount.unstakeBalance, getTokenMetadata("TUNC").decimals))} TUNC</span>
                            </div>
                            <div className="flex flex-row justify-between mb-0.5">
                    ...
                    <div className="w-full grid mt-3 grid-cols-2 lg:grid-rows-2 gap-2">
                        ...
                        <div
                          className="lg:h-16 xs:h-20 md:h-20 rounded-lg bg-darkGradientBg shadow-dark p-2.5 hover:bg-darkGradientHoverBg">
                            ...
                            <div className="lg:flex lg:justify-center lg:items-center">
                                <label className="text-base font-medium text-xREFColor">{formatNumber(parseTokenWithDecimals(poolInfo.totalStakeBalance, getTokenMetadata("TUNC").decimals))}</label>
                                <label className="text-xs ml-1.5 text-primaryText">TUNC</label>
                            </div>
                        </div>
                        <div
                          className="lg:h-16 xs:h-20 md:h-20 rounded-lg bg-darkGradientBg shadow-dark p-2.5 hover:bg-darkGradientHoverBg">
                            ...
                            <div className="lg:flex lg:justify-center lg:items-center">
                                <label className="text-base font-medium text-xREFColor">{formatNumber(parseTokenWithDecimals(poolInfo.totalReward, getTokenMetadata("TUNC").decimals))}</label>
                                <label className="text-xs ml-1.5 text-primaryText">TUNC</label>
                            </div>
                        </div>
                    </div>
