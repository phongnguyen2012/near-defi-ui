...
const FaucetPage = () => {
    ...
    const handleMaxButton = () => {
        let currentShared = parseTokenWithDecimals(accountBalanceShare, getTokenMetadata("TUNC").decimals);
        let maxSharePerAccount = parseTokenWithDecimals(faucetInfo.maxSharePerAccount, getTokenMetadata("TUNC").decimals)
    ...
    const isDisable = () => {
        let currentShared = parseTokenWithDecimals(accountBalanceShare, getTokenMetadata("TUNC").decimals);
        let maxSharePerAccount = parseTokenWithDecimals(faucetInfo.maxSharePerAccount, getTokenMetadata("TUNC").decimals);
        let totalBalance = parseTokenWithDecimals(faucetInfo.totalBalanceShare, getTokenMetadata("TUNC").decimals);
    ...
    const handleFaucet = async () => {
        ...
        try {
            // @ts-ignore
            await faucet(parseTokenAmount(faucetValue, getTokenMetadata("TUNC").decimals).toLocaleString('fullwide', {useGrouping:false}));
        } catch (e) {
            console.log("Error", e);
        } finally {
          ...
          <section className="w-full md:w-560px lg:w-560px xl:w-560px m-auto relative xs:px-2">
              ...
              <div className={"flex flex-col mt-5 justify-between"}>
                  <div className={"flex flex-row justify-between mb-2 bg-cardBg rounded-2xl p-5 w-full"}>
                      ...
                      <p className={"text-2xl text-white"}>{formatNumber(parseTokenWithDecimals(faucetInfo.totalBalanceShare, getTokenMetadata("TUNC").decimals))} TUNC</p>
                  </div>
              </div>
              <div className={"bg-cardBg rounded-2xl p-5 mb-2"}>
                  <div className={"input-form mt-5"}>
                      <p className="flex flex-row items-center text-primaryText mb-2">
                          ...
                          {formatNumber(parseTokenWithDecimals(accountBalanceShare, getTokenMetadata("TUNC").decimals))}
                      ...
                      <p className="text-xs text-primaryText mb-1">
                          {
                              accountBalanceShare == faucetInfo.maxSharePerAccount ?
                                <span>One account can get max {formatNumber(parseTokenWithDecimals(faucetInfo.maxSharePerAccount, getTokenMetadata("TUNC").decimals))} TUNC. You can not get more!</span>
                                :<span>One account can get max {formatNumber(parseTokenWithDecimals(faucetInfo.maxSharePerAccount, getTokenMetadata("TUNC").decimals))} TUNC. You can get more!</span>
                          }
                      </p>
              ...
              <div className="w-full grid mt-3 grid-cols-2 lg:grid-rows-2 gap-2">
                  ...
                  <div
                    className="lg:h-16 xs:h-20 md:h-20 rounded-lg bg-darkGradientBg shadow-dark p-2.5 hover:bg-darkGradientHoverBg">
                      ...
                      <div className="lg:flex lg:justify-center lg:items-center">
                          <label className="text-base font-medium text-xREFColor">{formatNumber(parseTokenWithDecimals(faucetInfo.totalShared, getTokenMetadata("TUNC").decimals))}</label>
